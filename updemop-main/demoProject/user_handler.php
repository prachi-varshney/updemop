...
class User
{
    function tableList($db, $data)
    {
        ...
        $start_from = ($data['page'] - 1) * $data['records_per_page'];
    
        $query = "SELECT * FROM user_master WHERE $condition $sorting LIMIT $start_from, $records_per_page";


        $query_total = "SELECT COUNT(*) AS total FROM user_master WHERE $condition";
        // $query = "SELECT * FROM user_master WHERE $condition $sorting";
        // print_r($query);die;
        $recordData = $db->getData($query_total);
        $rec_total = (int) $recordData['data'][0]['total'];
        $result = $db->getData($query);
        $total_pages = ceil($rec_total / $data['records_per_page']);
        $html = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['type'])) {
        ...
        if (!empty($type) && $type == 'LIST') {
            echo json_encode($user->tableList($db, $_POST));
